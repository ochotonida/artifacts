...
public abstract class ArtifactItem extends Item {
    ...
    public void appendHoverText(ItemStack stack, Level world, List<Component> tooltipList, TooltipFlag flags) {
        if (ArtifactsForge.CONFIG.client.showTooltips) {
    ...
    protected void addTooltip(List<MutableComponent> tooltip) {
        if (isCosmetic()) {
            tooltip.add(Component.translatable("%s.tooltip.cosmetic".formatted(ArtifactsForge.MOD_ID)).withStyle(ChatFormatting.ITALIC));
        } else {
    ...
    protected void addEffectsTooltip(List<MutableComponent> tooltip) {
        tooltip.add(Component.translatable("%s.tooltip.item.%s".formatted(ArtifactsForge.MOD_ID, getTooltipItemName())));
    }

    protected MutableComponent tooltipLine(String lineId, Object... args) {
        return Component.translatable("%s.tooltip.item.%s.%s".formatted(ArtifactsForge.MOD_ID, getTooltipItemName(), lineId), args);
    }
